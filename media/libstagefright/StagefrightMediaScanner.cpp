...
static void flac_metadata_cb(
    ...
    if (metadata->type == FLAC__METADATA_TYPE_STREAMINFO) {
        FLAC__uint64 duration = 1000 * metadata->data.stream_info.total_samples /
        ...
        for (uint32_t i = 0; i < metadata->data.vorbis_comment.num_comments; i++) {
            ...
            if (value) {
                int keylen = value++ - entry;
                char key[keylen + 1];
                strncpy(key, entry, keylen);
                key[keylen] = '\0';
                LOGD("flac_metadata_cb: key: %s, value: %s", key, value);
                if (!client->addStringTag(key, value))
                    return;
            }
        }
    }
...
static status_t HandleFLAC(const char *filename, MediaScannerClient *client) {
    ...
    init_status = FLAC__stream_decoder_init_file(decoder, filename, flac_write_cb,
            flac_metadata_cb, flac_error_cb, client);
